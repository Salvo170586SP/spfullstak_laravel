<x-app-layout>
    <div class="container mx-auto p-4">
        <div class="flex flex-col">
            <div class="col">
                <h1 class="text-5xl mb-2 font-bold text-white">Disegni</h1>
            </div>
            <div class="col">
            ...
            <div class="col mx-auto lg:w-5/6">
                <form action="{{ route('admin.draws.store') }}" method="post" enctype="multipart/form-data">
                    @csrf
            ...
            <div class="col block md:hidden my-10 mx-auto w-full">
                <form action="{{ route('admin.draws.index') }}" method="post" class="flex items-center">
                    @csrf
                    ...
                    <a href="{{ route('admin.draws.index') }}" class="font-bold text-white rounded my-5 p-2 bg-red-500 ms-2" type="submit">Reset</a>
                </form>
                <div class="max-w-xxl dark:bg-gray-600 dark:text-white py-4 px-8 bg-white shadow-lg rounded-lg" style=" overflow: auto">
                    <div class="py-5">
                        ...
                        <div class="flex justify-between items-center py-3">
                            <div>
                                @if($draw->img_url)
                                <img class="rounded-md my-3" width="90" src="{{ asset('storage/'. $draw->img_url) }}" alt="{{$draw->title }}">
                                @else
                                ...
                                @endif
                                <div class="text-2mdmb-2 font-bold text-white">Nome: {{ $draw->title }}</div>
                                <div class="text-2md mb-2 font-bold text-white">Url: {{ $draw->url }}</div>
                            </div>
                            <div class="flex item-center justify-center">
                                <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                    <a href="{{ route('admin.draws.edit', $draw->id) }}">
                                        <i class="fa-solid fa-pencil"></i>
                                    </a>
                                </div>
                                <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                    <form class="delete-form" action="{{ route('admin.draws.destroy', $draw->id) }}" method="post" data-title="{{$draw->title}}">
                                        @csrf
                                        @method('delete')
                ...
                <div class="overflow-x-auto">
                    <div class="min-w-screen  my-9 flex items-center justify-center  font-sans overflow-hidden">
                        <div class="w-full lg:w-5/6">
                            <form action="{{ route('admin.draws.index') }}" method="get" class="flex items-center">
